...
// ============================================
// 🔥 HIMO AI - COMPLETE VERSION (CommonJS)
// ============================================
...
💡 Example: "GST 1000 18"`;
    ...
    getSpace(q) {
        ...
        const planets = {
            ...
            uranus: '🪐 Uranus: Side rotation, 27 moons, -197°C',
            neptune: '🪐 Neptune: Strongest winds, 16 moons, -201°C'
        };
        for (const p of Object.keys(planets)) {
            if (ql.includes(p)) return planets[p];
        }
        if (ql.includes('sun')) return "☀️ Sun: G-type star, 1,391,000 km diameter, 5,500°C surface";
        if (ql.includes('black hole')) return "🕳️ Black Hole: Gravity so strong light can't escape!";
...
print(p.greet())`;
    ...
    getConversation(q) {
        ...
        if (ql.match(/what is your name|who are you|kaun ho tum|your name/i)) {
            return "🤖 I'm Himo AI! Your intelligent assistant!";
        }
...
✅ ANYTHING else!`;
        ...
        if (ql.match(/i love you|love you|i like you/i)) {
        ...
        if (ql.match(/thank|thanks|dhanyawad|shukriya/i)) {
    ...
    getGames(q) {
        ...
        const games = {
            ...
            chess: '♟️ Chess: 2 players, Strategy, Rules: Checkmate, 16 pieces each',
...
Surface Area = 4πr² = ${(4 * Math.PI * r * r).toFixed(2)}`;
    ...
    async process(input) {
        ...
        // Weather
        if (q.toLowerCase().match(/weather|temperature|humidity|rain|monsoon|mausam|aaj ka|kitna garam|delhi|mumbai|chennai|kolkata|bangalore|hyderabad|jaipur|pune/i)) {
...
🌤️ Weather: "Weather in Delhi", "Mumbai temperature"
...
🍽️ Food: "Moong Dal", "Apple nutrition"
📚 Knowledge: "Meaning of [word]", "What is [topic]?"
